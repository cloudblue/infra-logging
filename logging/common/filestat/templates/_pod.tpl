...
{{- end }}
serviceAccountName: {{ include "filestat.serviceAccountName" . }}
securityContext:
...
containers:
  ...
  {{- end }}
    ...
    volumeMounts:
      ...
      {{- print "- name: filestat-custom-cm-" $key  | nindent 6 }}
        {{- print "mountPath: /etc/filestat/" $key ".d"  | nindent 8 }}
      {{- end }}
volumes:
  {{- toYaml .Values.volumes | nindent 2 }}
  {{- range $key := .Values.configMapConfigs }}
  {{- print "- name: filestat-custom-cm-" $key  | nindent 2 }}
    configMap:
